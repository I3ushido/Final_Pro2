...
def processing(path_location):
    ...
    #     json.dump(type_car, f)

    # todo write file type !! Dis
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        self.button_upload_data = QPushButton('Upload Information', self)
        self.button_upload_data.resize(150, 50)
        self.button_upload_data.move(370, 270)
        self.button_upload_data.clicked.connect(self.upload_information)
        ...
        # video_name
        self.video_name = QLabel('VDO Name : ', self)
        self.video_name.resize(100, 20)
        self.video_name.move(370, 330)
        self.video_name_box = QLineEdit(self)
        self.video_name_box.setText('Project2')
        self.video_name_box.resize(70, 20)
        self.video_name_box.move(450, 330)

        # Create Data Base
        self.button_upload_data = QPushButton('Create Database', self)
        self.button_upload_data.resize(150, 25)
        self.button_upload_data.move(370, 360)
        self.button_upload_data.clicked.connect(self.create_database)
    ...
    def create_database(self):
        database_name = self.video_name_box.text()
        info = {"video_time": database_name}
        response = reqs.post('http://206.189.46.191:443/inserts', json=info)
    ...
    def upload_information(self):
        ...
        for x in f:
            ...
            data = {"car_category": datas[0], "car_colors": datas[1], "car_brand": datas[2], "speed": datas3[0],
                    "car_image": '/img/car_' + str(count) + '.png'}
            # app_data = json.dumps(data)
            print(data)
            response = reqs.post('http://206.189.46.191:443/inserts_data', json=data)
            print(response.text), 200
            count += 1

    def upload_image(self):
        image_list = []
        for filename in glob.glob('Save/*.png'):
    ...
    def check_Color(self, pic_color):
        ...
        # with open("output.txt", "a") as f:
        #     json.dump(convert_name_color, f)
        f = open("output.txt", "a")
        f.write(' ' + convert_name_color)
        f.close()
        ...
        self.label_logos.setText('Brand : ' + logo_name)

        f = open("output.txt", "a")
        f.write(' ' + logo_name)
        f.close()

        # # todo write file logo name
    ...
    def play_Video(self, name):
        ...
        while True:
                            ...
                            if 500 <= cx <= 950:  # cx 1200 if cx >= 500 and cx <= 950:
                                ...
                                if self.position[0] <= cx <= self.position[0] + 375:  # if (cx >= 500) and (cx <= 875):
                                    ...
                                    if self.position[1] < cy < self.position[1] + 30:  # Line Capture;
                                        ...
                                        typeof_car = processing(save_file)
                                        self.label_type.setText('Type : ' + typeof_car)
                                        if flag == 0:
                                            ...
                                            cal_speed = self.position[4] / (time_lock) + 13
                                            print('Speed : ', cal_speed)
                                            self.label_speed.setText('Speed : ' + str(cal_speed))
                                            # with open("output.txt", "a") as f:
                                            #     json.dump(str(cal_speed) + '\n', f)

                                            f = open("output.txt", "a")
                                            f.write(' ' + str(cal_speed) + '\n')
                                            f.close()
            ...
            # cv2.rectangle(image, (340, 420),(570, 670),(30, 30, 220), 2)
