...
def read_images(fold_path):
    ...
    return imgs
##
root_dir = os.path.abspath('Pre_dataset/')
data  = []
label = []
...
Xtrain = X_train.reshape([-1, 96, 96, 1])
Xtest = X_test.reshape([-1, 96, 96, 1])
...
model.fit({'input': Xtrain}, {'target': y_train}, n_epoch=50,
           validation_set=({'input': Xtest}, {'target': y_test}),
           snapshot_step=10, show_metric=True, run_id='Car_Type')
model.save('modeltestDataV3.cnn')
