                ...
                for i in range(len(contours)):
                    if hierarchy[0, i, 3] == -1:
                        ...
                        if minarea < area < maxarea:
                            ...
                            cy = int(M['m01'] / M['m00'])
                            # Speed_check
                            if cx >= 560 and cx <= 780 and cy >= 144 and cy <= 480:
                                sed_time = time.time() - start_time
                                ##                        print('start timer: ', sed_time)
                                timer.append(sed_time)
                                if cy >= 460 and cy <= 480:
                                    elapsed_time = time.time() - start_time
                                    end = datetime.datetime.now()
                                    elapsed = end - start
                                    print('CX: ', cx, 'CY: ', cy, 'ori:', elapsed_time, 'Time:',
                                          time.strftime("%H:%M:%S", time.gmtime(elapsed_time)), ' microseconds: ',
                                          elapsed.microseconds)
                                    timer.append(sed_time)
                                    print('time st: ', timer[0], 'last: ', timer[-1])
                                    time_lock = timer[-1] - timer[0]
                                    print('time_lock : ', time_lock)
                                    print(len(timer))
                                    try:
                                        print('Speed : ', 20 / time_lock, 'Km/h')
                                    except:
                                        print("speed error.")
